package dhanar10.irpropplusneuralnetwork;

public class IRpropPlusNeuralNetwork {
	...
	private double pmse = Double.MAX_VALUE;
	...
	public static void main(String[] args) {
		...
		while (irpropplus.canTrain()) {
			irpropplus.train(data);
			
			System.out.println(irpropplus.getEpoch() + "\t" + irpropplus.getMse());
		}
		
		System.out.println();
		
		for (int i = 0; i < data.length; i++) {
			double output[] = irpropplus.calculate(data[i]);
		...
		System.exit(irpropplus.getMse() < irpropplus.getTargetMse() ? 0 : 1);
	}

	public IRpropPlusNeuralNetwork(int input, int hidden, int output) {
		...
		for (int i = 0; i < wInputHidden[0].length; i++) {
			for (int j = 0; j < wInputHidden.length; j++) {
				...
				if (change > 0) {
					...
					if (mse > pmse) {
						dwInputHidden[j][i] = -dwInputHidden[j][i];
						wInputHidden[j][i] += dwInputHidden[j][i];
					}
					gpInputHidden[j][i] = 0;
				}
		...
		for (int i = 0; i < wHiddenOutput[0].length; i++) {
			for (int j = 0; j < wHiddenOutput.length; j++) {
				...
				if (change > 0) {
					...
					if (mse > pmse) {
						dwHiddenOutput[j][i] = -dwHiddenOutput[j][i];
						wHiddenOutput[j][i] += dwHiddenOutput[j][i];
					}
